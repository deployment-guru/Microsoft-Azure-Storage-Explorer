...
Try {
    ...
    [String]$appName = 'Azure Storage Explorer'
    [String]$appVersion = ''
    [String]$appArch = ''
    [String]$appLang = 'DE'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		$AnyFilePath = Get-ChildItem -Path "$dirFiles" -Include StorageExplorer.exe -File -Recurse -ErrorAction SilentlyContinue
        If($AnyFilePath.Exists) {
			Execute-Process -Path "$AnyFilePath" -Parameters "/VERYSILENT /NORESTART /ALLUSERS /LOG=$configToolkitLogDir\AzureStorageExplorer_Install.log" -WindowStyle Hidden
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        Show-InstallationWelcome -CloseApps 'StorageExplorer' #-CloseAppsCountdown 60
		...
		$SourceCache = Get-InstalledApplication -Name 'Azure Storage Explorer'    
        ForEach ($item in $SourceCache) {
			If($item.UninstallString) {
				$UninstPath = $item.UninstallString -replace '"', ''       
				If(Test-Path -Path $UninstPath) {
					Execute-Process -Path $UninstPath -Parameters "/VERYSILENT /NORESTART /LOG=$configToolkitLogDir\AzureStorageExplorer_Uninstall.log"
					Start-Sleep -Seconds 5
				}
			}
        }
